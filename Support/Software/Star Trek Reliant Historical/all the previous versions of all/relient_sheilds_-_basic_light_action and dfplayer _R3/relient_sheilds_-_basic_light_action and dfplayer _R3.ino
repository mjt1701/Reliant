/*
  ...
  Rev3.2 clean up variables etc from 3.x  
...
enum sheildStates {
  onePlaying,      // 1 mp3-1 playing
  twoStarted,      // 2 mp3-2 playing only ie need to wait for time to start sheilds down
  goingDown,       // 3 mp3-2 playing and sheilds going down
  playingOut,      // 4 last light is out, mp3-2 still playing (or change to nothing and handle after mp3 is done)
  waitToStartAgain // reset and start over
};

enum sheildStates shldState;
...
void loop() {
  ...
  Serial.println(mode);
  //Serial.print(" Case number 'in Switch': ");
  //Serial.println(mode);
  ...
  switch (shldState) {
    ...
    case playingOut:  // sheilds have gone down, mp3-2 may be playing , then reset to start again
        //DO WHATEVER TO RESET AND START OVER
        //MAKE IT A FUNCTION WITH SETUP()
        delay(20000);
...
Serial.println("after delay for next go around ");
