...
void loop()
{
	...
	case onePlaying: // mp3-1 playing
		...
		if (digitalRead(buttonPin) == buttonActivated) // todo add comment what this doing in english
		{
	...
	case goingDown: // mp3-2 playing and sheilds going down
		...
		if (ledNextTurnOffNum == sheildLEDnum) // todo add comment what this doing in english
		{									   // todo last light is out (or change to nothing and handle after mp3 is done)
			ledNextTurnOffNum = 0;
			ledOffTime = millis();
		...
		{											 // Turn off lights
			if (millis() - ledOffTime > ledOffDelay) // todo add comment what this doing in english
			{
	...
	case playingOut: // sheilds have gone down, mp3-2 may be playing , then reset to start again
		...
		if (millis() - ledOffTime > runOutMP3s2Time) // todo add comment what this doing in english
		{
	...
	case waitToStartAgain:
		if (millis() - mp3s2OutTime > resetDelay) // todo add comment what this doing in english
		{
