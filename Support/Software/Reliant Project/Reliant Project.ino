...
const unsigned long mp3File3_2Delay = 26000; // Trigger delay after File start for next action
...
void setup()
{
	...
	// Initialize the LEDs and dfPlayer,  turn each strip on and off
...
void loop()
{
	...
	case initial: // anything that needs done ate each loop on turn on  //? delete case?? move to default
				  //	shldState = buttonPressFile1;  //!change this back to uncommented
				  // todo move lightning() here
		lightning();
...
void clearLEDs() //! Can this be deleted??????
{
...
void resetLeds() //! Can this be deleted??????
{
...
const int sec_count = 7; // the maximum "width" of each lightning sequence
const int seq_count = 5; // the maximum "duration" of each lightning sequence
const int NUM_SECTIONS = 4;
const int SECTION_LEN = 2;

void lightning()
{
	// randomly select the color of the current lightning sequence
	int r = random(90, 255);
	int g = random(90, 255); // - 100;
	int b = random(90, 255);
	...
	/*
					Serial.print("seq_max: ");
					Serial.println(seq_max);

					Serial.print("seq_order[i]: ");
					for (int p=0; p<sec_count; p++)
					{
					Serial.println(seq_order[p]);
					delay(4000);
	}
	*/
	...
	Serial.println(NUM_SECTIONS);

	Serial.print("seq_max: ");
	Serial.println(seq_max);

	Serial.print("seq_start: ");
	Serial.println(seq_start);
...

	{
		...
		for (int k = 0; k < SECTION_LEN; k++)
		{
			...
			shipLEDstrip.setPixelColor(pix_cur, shipLEDstrip.Color(r, g, b));

			// turn off the LEDs of the previous section
			if (j > 0)
			{
				int pix_prv = ((seq_start + seq_order[j - 1]) * SECTION_LEN) + k;
				shipLEDstrip.setPixelColor(pix_prv, shipLEDstrip.Color(0, 0, 0));
			}
		}
